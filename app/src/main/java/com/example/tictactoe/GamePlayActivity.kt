...
class GamePlayActivity : AppCompatActivity() {
    ...
    private fun playNow(buttonSelected: Button, currCell: Int) {
        var audio = MediaPlayer.create(this, R.raw.swap_button_sound)
        if (activeUser == 1) {
    ...
    private fun robot() {
        ...
        if(emptyCells.contains(rnd)){
            ...
            emptyCells.add(rnd)
            val audio = MediaPlayer.create(this,R.raw.swap_button_sound)
            audio.start()
    ...
    private fun checkWinner(): Int {
        val audio = MediaPlayer.create(this, R.raw.start_game_sound)
        if (
            // prostopadle
